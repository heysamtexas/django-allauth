...
- OAuth2/OIDC: When setting up multiple apps for the same provider, you can now
  ...
  settings.
- Facebook login: Facebook [Limited Login](https://developers.facebook.com/docs/facebook-login/limited-login) is now supported via the Headless API. When you have a Limited Login JWT obtained from the iOS SDK, you can use the Headless "provider token" flow to login with it.
